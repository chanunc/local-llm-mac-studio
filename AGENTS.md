...
- **mlx-openai-server / mlx-lm server** (port 8000, OpenAI only) — **current production main (2026-05-06)**: `lmstudio-community/gemma-4-31B-it-MLX-6bit` served via `python -m mlx_lm server` (direct path, not mlx-openai-server YAML; **20.4 tok/s, browse 12.33 s / search 35.55 s**, thinking mode ON). MTP drafter cached but `gemma4_assistant` arch not yet in mlx-lm 0.31.3. `mlx-openai-server` also available with trie-based prompt caching, speculative decoding, and multi-model YAML config.
