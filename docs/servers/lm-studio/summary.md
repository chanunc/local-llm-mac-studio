...
- **modelKey prefix collisions silently change the API id.** `lms load <modelKey>` without `--identifier` registers the running model under that bare modelKey, so any sibling sharing the same prefix (e.g. `gemma-4-26b-a4b-it` vs `gemma-4-26b-a4b-it-uncensored`) makes the resulting API id ambiguous and resolution depends on lms's pick order. The matching `lms ps` output even prints a warning (`W 2 models match the provided model key on the same device`) but proceeds. **Always pass `--identifier <stable-id>`** for any model whose modelKey can collide with another in `lms ls`, and pin that stable id in the matching `configs/clients/lm-studio/*.json` templates. If the live identifier ever drifts from the template (e.g. someone reloaded without `--identifier`), `chk_llm_macstu.py` will surface the mismatch — `lms ps` will report the bare modelKey while OpenCode keeps requesting the templated id, and LM Studio will reject every request. Reload with the documented `--identifier` to converge. Hit 2026-05-07 with `gemma-4-26b-a4b-q8` / `gemma-4-26b-a4b-it`.
