...
| RotorQuant / TurboQuant / PlanarQuant KV-cache experiments (port 8099) | `llama-cpp-turboquant` | `unsloth/Qwen3.6-35B-A3B-UD-Q6_K.gguf` + `--cache-type-k iso3 --cache-type-v iso3` | Provisional sidecar built from `johndpope/llama-cpp-turboquant` `feature/planarquant-kv-cache` fork. First Apple-Silicon path for RotorQuant. Smoke 5/5; **decode 46.3 tok/s @ 512 (2.27× Gemma 4 baseline)**, but cold prefill regression at 32 K+ (>600 s) and OpenCode browse 20.5 s / search 151.18 s (slower than Gemma 4 in agent loops because tool-result injection forces cold prefill per turn). Workload-fit: short-context / multi-turn loops where prompt prefix re-caches. See [`docs/servers/llama-cpp-turboquant/summary.md`](servers/llama-cpp-turboquant/summary.md). |
