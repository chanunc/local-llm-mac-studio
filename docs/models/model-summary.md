...
#### Potential unblocking (investigated 2026-04-20)

Deeper GitHub investigation shows the TQ loader is shipped, just via a different distribution than the HF model cards document:

- **`jjang-ai/vmlx` v1.3.49+ bundles `jang_tools.load_jangtq` and the `turboquant/*kernel*` Metal kernels inside its own wheel**, shadowing the public PyPI `jang` package on import. Confirmed by maintainer in [vmlx#72](https://github.com/jjang-ai/vmlx/issues/72): "The TurboQuant fast path that actually understands `JANGQ-AI/MiniMax-M2.7-JANGTQ` landed in **1.3.49**... vMLX loads it natively via `jang_tools.load_jangtq.load_jangtq_model`, installs `TurboQuantLinear` / `TurboQuantSwitchLinear` modules with all P3/P15/P17/P18 Metal-kernel optimizations, and decodes at **~42 tok/s steady-state on Mac Studio M3 Ultra**."
- **vmlx `main` is at 1.3.65**, distributed as `pip install vmlx` / `uv tool install vmlx` / `pipx install vmlx`. Apache-2.0, no bundled-Python gate, OpenAI + Anthropic + Ollama-compatible API server.
- **vmlx CHANGELOG explicitly calls out CRACK-family TQ models as shipped-clean**: "MiniMax-M2.7-JANGTQ-CRACK: full 4-cell reasoning matrix ship-clean" and "Qwen3.6-35B-A3B-JANGTQ2: 7 scenarios — multi-turn, hybrid TQ KV cache, VL single/multi-image, L2 disk cache bit-exact round-trip, video fallback, sustained RAM (+0.00 GB growth over 20 mixed requests)."
- **Why the 2026-04-17 attempt still failed**: `vmlx[jang]==1.3.61` was pinned against a stale `jang` dep that shadowed the bundled loader — the dequant-and-requant fallback (gibberish loops) triggered instead of the native TQ path. A clean venv with vmlx 1.3.49+ and **no competing `jang-tools` on `sys.path`** is required for the bundled loader to win import resolution.
- **Upstream `jjang-ai/jangq` issue [#5](https://github.com/jjang-ai/jangq/issues/5)** (filed 2026-04-19 by `mshicom`) is the authoritative public record of the block on the PyPI path and is still open/unanswered — the loader stubs on the `jang-spec-plan5-bundle-python-validation` branch are incomplete (missing `turboquant.tq_kernel`, `hadamard_kernel`, `gather_tq_kernel`, `fused_gate_up_kernel`). So the PyPI `jang` path remains blocked; the vmlx path is the only working route.
- **Incompatible flags**: per [vmlx#81](https://github.com/jjang-ai/vmlx/issues/81), JANGTQ models must load via the native fast path only — `--smelt` and `--flash-moe` do not support `TurboQuantLinear` and will raise a `vmlx#81` ValueError on `weight_format=mxtq`.

**Path to unblock**: install `vmlx>=1.3.49` (ideally 1.3.65) in a clean venv (e.g. `~/vmlx-env/`), smoke-test `MiniMax-M2.7-JANGTQ-CRACK` and/or re-download `Qwen3.6-35B-A3B-JANGTQ4`, compare tok/s against the 42 tok/s baseline, and — if viable — add a fourth server-switch to CLAUDE.md alongside `vllm-mlx` / `mlx-openai-server` / `oMLX`. Single-vendor dependency risk remains until `jjang-ai/jangq#5` lands the loader + kernels in the public `jang-tools` package.
