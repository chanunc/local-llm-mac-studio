...
|:------|:-------|:-----------------|:----------------------------:|:------|
| IBM Granite 4.1 30B Q8_0 | llmster | **5/5** | 10.37 s | LM Studio runtime handles Granite tool format natively; no parser flags required. Single-call latencies 1.13-2.99 s. Raw: [`api-tool-test.json`](granite-4.1-30b-q8/api-tool-test.json) |
...
| TrevorJS Gemma 4 26B A4B Uncensored Q8_0 | llmster | **5/5** | **2.14 s** 🥇 | LM Studio GGUF runtime; non-thinking MoE — single-call latencies 0.29–0.83 s (fastest in this table). Raw: [`api-tool-test.json`](gemma4-26b-a4b-trevorjs-uncen/api-tool-test.json) |
...
**Server:** llmster / LM Studio headless on port 1234, GGUF runtime. Loaded with `--identifier granite-4.1-30b-q8 --context-length 65536`. Raw JSON: [`api-tool-test.json`](granite-4.1-30b-q8/api-tool-test.json).
...
**Pass rate: 5/5.** Tool calls are well-formed. LM Studio handles Granite 4.1 tool-call format natively — no `--tool-call-parser` flag required at load time.
...
- Dense 30B at 24.8 tok/s; multi-turn loop is ~5× slower than TrevorJS Gemma 4 26B A4B MoE (2.14 s) but similar to Gemma 4 31B-it (9.8 s).
- LM Studio guardrail `mode: "high"` blocks this load — must toggle to `"off"` before `lms load`, restore after.
