...
# speculative-decoding sidecar on port 8100 (NOT 8000/1234/8098/8099). Built
# from am17an/llama.cpp@mtp-clean (PR #22673 — unmerged in upstream llama.cpp).
...
| **[lm-studio](docs/servers/lm-studio/summary.md)** ([LM Studio](https://lmstudio.ai/) headless, :1234) | ⚡ Fastest agent loop · uncensored compliance | Standard MLX / GGUF | OpenAI | **Stopped 2026-05-15 (speed-first fallback).** Prior production main (2026-05-14 → 2026-05-15): `prithivMLmods/Q3.6-27B-GLM-5.1-DA-GGUF` Q4_K_M (Apache 2.0, dense 27B + ViT, think-on, 15.41 GiB resident, decode 31 tok/s @ 512, refusal 9/10 @ 1024 tok, OpenCode browse **11.62 s** / search **19.47 s** @ 2 turns, `webfetch` fired) — faster on agent loops than the current `llama-cpp-mtp` MTP main; restart with the one-liner in [`docs/current.md`](docs/current.md). Also on disk: TrevorJS Gemma 4 31B-it Uncensored Q4_K_M (browse 6.63 s warm / search 30.81 s, harness 6–7/10 / manual 10/10), lmstudio-community Gemma 4 26B A4B-it Q8_0 (prior main, MoE 4B-active 87 tok/s), unsloth Qwen3.6-35B-A3B UD-Q6, Granite 4.1 30B Q8, Gemma 4 31B-it MLX 6-bit (browse **5.11 s 🥇 thinking OFF**), TrevorJS Gemma 4 26B A4B uncensored, DavidAU Heretic family, prithivMLmods Aggressive. No JANG/JANGTQ/bailing_hybrid. |
...
| **[llama-cpp-mtp](docs/servers/llama-cpp-mtp/summary.md)** (**CURRENT MAIN 2026-05-15**, :8100) | 🟢 84–89 % MTP draft acceptance | Single GGUF with embedded MTP draft heads | OpenAI | **Current production main (2026-05-15): `unsloth/Qwen3.6-27B-MTP-GGUF:UD-Q6_K_XL`** — Qwen3.6 Multi-Token Prediction self-drafting speculative decoding via [`am17an/llama.cpp@mtp-clean`](https://github.com/am17an/llama.cpp/tree/mtp-clean) (PR #22673, unmerged upstream). Dense 27 B + MTP heads, Unsloth Dynamic 6-bit, 26 GB. Decode 22.9 → 20.0 tok/s @ 414 → 29 128 input tokens with 86 % avg draft acceptance, smoke **5/5** single-call + 3-turn multi 21.92 s, OpenCode browse 35.98 s / search 35.24 s. Patch-free apart from the build; only path for Qwen3.6 MTP on Apple Silicon today. `-np > 1` and `--mmproj` unsupported with MTP active. Prior lm-studio Q4_K_M main was faster on agent loops — kept as the speed-first fallback. See [bench](docs/models/benchmarks/logs/qwen36-27b-mtp/) |
...
Current production main: **`llama-cpp-mtp`** (custom `llama-server` from [`am17an/llama.cpp@mtp-clean`](https://github.com/am17an/llama.cpp/tree/mtp-clean) PR #22673) serving `qwen3.6-27b-mtp-ud-q6kxl` (`unsloth/Qwen3.6-27B-MTP-GGUF:UD-Q6_K_XL` — dense 27 B Qwen3.6 with Multi-Token Prediction self-drafting heads baked into a single GGUF, Unsloth Dynamic 2.0 6-bit, 26 GB on disk / ~28 GB resident, Apache 2.0 base) on port **8100** (LAN-bound `0.0.0.0`, deployed 2026-05-15). Tool calls + reasoning channel dispatch from the GGUF's embedded chat template via `--jinja` (no `--tool-call-parser` flag exists on this build); `<think>...</think>` goes into `reasoning_content` via `--reasoning on`. Launch enables MTP self-drafting with `--spec-type draft-mtp --spec-draft-n-max 2`; **84–89 % MTP draft acceptance** measured across 414 → 29 128 input tokens (upper end of Unsloth's claim). API direct: smoke 5/5 single-call + multi-turn 21.92 s; decode 22.9 / 22.3 / 22.0 / 20.0 tok/s @ 414 / 3 648 / 7 274 / 29 128 input tokens; warm TTFT 0.15–0.21 s. **Agent loop** (OpenCode 1.14.50, 1 warmup + 3 measured): browse **35.98 s wall / 35.2 s LLM**, search **35.24 s wall / 34.45 s LLM**, 2 turns + `webfetch` fired on every measured run. **Slower than the prior lm-studio Q4_K_M main** (browse 11.62 s / search 19.47 s) — MTP's ~1.5–2× speedup doesn't close the dense-27 B-6-bit weight-bundle gap; if speed matters more than the MTP experiment, the prior GLM-5.1-DA main stays on disk as the speed-first fallback (one-liner in [`docs/current.md`](docs/current.md)). `-np > 1` and `--mmproj` unsupported with MTP active — no multi-pipeline, no vision. Prior main (2026-05-14 → 2026-05-15): lm-studio + prithivMLmods Q3.6-27B-GLM-5.1-DA Q4_K_M on port 1234 (stopped during this switch; restartable as the speed-first fallback).
...
| [TrevorJS Gemma 4 26B A4B Uncensored Q8_0](docs/models/uncen-model/gemma4-26b-a4b-trevorjs-uncen-benchmark.md) | MoE 26B/4B active | 25 | 65K | Prior lm-studio main (2026-05-03) — EGA abliteration, 87.6 tok/s, browse 2.93 s 🥇 (no scaffolding needed), 8/10 compliance — uncensored counterpart to current main |
