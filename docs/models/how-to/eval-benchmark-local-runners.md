...
How to run **MMLU**, **MMLU-Pro**, **TruthfulQA**, **HarmBench**, and **mlabonne/harmful_behaviors** against this stack's OpenAI-compatible endpoints (`lm-studio :1234`, `vmlx :8000`, etc.) without depending on hosted-only services or an OpenAI API key.
...
# Full MMLU against lm-studio (note: requires working /v1/completions logprobs)
...
| Time on lm-studio (35B MoE @ ~50 tok/s) | Full ~6–10 hr · `--limit 200` ~5–10 min |
...
| Time on lm-studio | Full 12k q ~6–12 hr · single category ~1 hr · `subset=0.05` ~30 min |
...
| Time on lm-studio | 817 q × 2 tasks ≈ 15–30 min |
...
| Time on lm-studio | 400 behaviors × ~30 s gen + ~5 s classify ≈ 3–4 hr · validation subset (~80) ≈ 40 min |
...
The current lm-studio main (`prithivMLmods Aggressive Q6_K`) is **not** suitable as a judge — it has been refusal-tuned out of, so it scores everything as "complied". The right pattern:
...
### First-week MVP plan for lm-studio prithivMLmods
...
|:-------|:-------------------------------|:------|
| lm-studio (LM Studio) | ⚠ Unreliable — [issue #2934](https://github.com/EleutherAI/lm-evaluation-harness/issues/2934) | Falls back to generative scoring silently. Verify before every benchmark run. |
...
For MMLU on lm-studio specifically, you may need to either (a) accept the generative-scoring fallback (and re-baseline your numbers against community generative-MMLU scores, not loglikelihood-MMLU scores), or (b) load the same model on vmlx for the MMLU pass only.
