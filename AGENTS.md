...
**lm-studio configs** (`configs/clients/lm-studio/`) ship `opencode.json` and `openclaw-provider.json` (added 2026-05-06 alongside the Granite 4.1 30B production-main switch). `claude-code-settings.json`, `pi-models.json`, `qwen-code-settings.json` remain deferred — backfill if/when those clients are pointed at lm-studio for sustained use.
