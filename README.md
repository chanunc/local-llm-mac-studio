...
# **AGENT LOOP BROKEN on OpenCode 1.14.48** — 0/6 runs emit AI SDK 5 events.
# For a working agent main, restore TrevorJS Gemma 4 31B-it Uncensored Q4_K_M fallback
# (`lms load gemma-4-31b-it-uncensored --identifier gemma4-31b-it-uncensored-trevorjs-q4km -y`).
...
| **[lm-studio](docs/servers/lm-studio/summary.md)** ([LM Studio](https://lmstudio.ai/) headless, :1234) | ⚡ Fastest agent loop · uncensored compliance | Standard MLX / GGUF | OpenAI | **Current production main (2026-05-14): `prithivMLmods/Q3.6-27B-GLM-5.1-DA-GGUF` Q4_K_M** (Apache 2.0, dense 27B + ViT, think-on, 15.41 GiB resident, decode 31 tok/s @ 512, refusal 9/10 @ 1024 tok). **Agent loop broken on OpenCode 1.14.48** — API harness passes 5/5, OpenCode emits zero AI SDK 5 events (see [bench](docs/models/uncen-model/qwen36-27b-glm51-da-benchmark.md)). Restore TrevorJS Gemma 4 31B-it Uncensored Q4_K_M (browse 6.63 s warm / search 30.81 s, harness 6–7/10 / manual 10/10) for a working agent main — still on disk. Also on disk: lmstudio-community Gemma 4 26B A4B-it Q8_0 (prior main, MoE 4B-active 87 tok/s), unsloth Qwen3.6-35B-A3B UD-Q6, Granite 4.1 30B Q8, Gemma 4 31B-it MLX 6-bit (browse **5.11 s 🥇 thinking OFF**), TrevorJS Gemma 4 26B A4B uncensored, DavidAU Heretic family, prithivMLmods Aggressive. No JANG/JANGTQ/bailing_hybrid. |
...
| **[vmlx-swift-lm](docs/servers/vmlx-swift-lm/summary.md)** (provisional, :1337) | 🔴 HTTP path 7-8 tok/s (regression) | Single Swift-MLX (BF16 / MLX / JANG / JANGTQ2/4 / MXFP4) | OpenAI + Anthropic + Ollama | **Stopped 2026-05-14** (was production main 2026-05-12 → 2026-05-14 with `JANGQ-AI/ZAYA1-8B-JANGTQ4`; Osaurus killed in pre-bench hygiene when swapping to lm-studio main). [`osaurus-ai/vmlx-swift-lm`](https://github.com/osaurus-ai/vmlx-swift-lm) engine consumed via `brew install --cask osaurus`. Only Mac Studio runtime that natively supports Zyphra ZAYA1 (top-1 CCA + MoE), Hunyuan v3, and the MiniMax-M2.7 JANGTQ Hadamard kernel optimization. Built-in tool/reasoning parsers (no flags). **JANGTQ HTTP-path was regressed** at pin `b9da180` — the engine's `RunBench` reports 57.2 tok/s on M4 Max but the OpenAI HTTP path ran 7-8 tok/s because the cask was missing the `BatchEngine.generate` B=1 fast path and the JANGTQ Hadamard kernel. Fix queued in [Osaurus PR #1057](https://github.com/osaurus-ai/osaurus/issues/1057). See [bench](docs/models/benchmarks/zaya1-8b/) |
...
Current production main: **lm-studio (LM Studio headless)** serving `qwen3.6-27b-glm51-da-q4km` (prithivMLmods Q3.6-27B-GLM-5.1-DA Q4_K_M — dense 27 B Qwen3.6 + ViT, prithivMLmods abliteration on Qwen3.6-27B + GLM-5.1 reasoning-trace distillation, think-on, Apache 2.0, 15.41 GB on disk / 15.41 GiB resident) on port **1234** (LAN-bound `0.0.0.0`, deployed 2026-05-14). LM Studio's built-in parser converts Qwen XML to `tool_calls[]` (no `--tool-call-parser` flag). API direct: smoke 5/5 single-call + 3/3 multi-turn; throughput 31.0 / 29.7 / 29.3 / 26.7 tok/s @ 512 / 4 K / 8 K / 32 K; refusal 9/10 @ `max_tokens=1024`. **⚠ Agent loop unusable** — `bench_agent_tool_call.py` on OpenCode 1.14.48 returned `agent_turns=0 tool_calls=[]` with zero AI SDK 5 events parsed across 6 runs (same failure family as the prior ZAYA1-8B / Osaurus main). Layer is OpenCode's AI SDK stream parser, not LM Studio's emitter (curl direct works). Full failure triage + upstream-filing draft: [`docs/models/uncen-model/qwen36-27b-glm51-da-benchmark.md`](docs/models/uncen-model/qwen36-27b-glm51-da-benchmark.md). **For a working OpenCode main today**, reload the TrevorJS Gemma 4 31B-it Uncensored Q4_K_M fallback (browse 6.63 s warm / search 30.81 s) — still on disk, listed in [`docs/current.md`](docs/current.md) Fallbacks. Prior main (2026-05-12 → 2026-05-14): vmlx-swift-lm + Zyphra ZAYA1-8B JANGTQ4 on Osaurus :1337 (stopped; also agent-broken at engine pin `b9da180` pending Osaurus PR #1057).
...
|:--------------------------------------|:------------|----------:|--------:|:---------|
| [prithivMLmods Q3.6-27B-GLM-5.1-DA Q4_K_M](docs/models/uncen-model/qwen36-27b-glm51-da-benchmark.md) | Dense 27B + VL (think-on) | 15.41 | 65K | **Current production main (deployed 2026-05-14 on lm-studio port 1234)** — Apache 2.0, prithivMLmods abliteration + GLM-5.1 reasoning-trace distillation, decode 31 / 30 / 29 / 27 tok/s @ 512 / 4K / 8K / 32K, smoke 5/5 + multi-turn 3/3 at API layer, refusal 9/10 @ 1024 tok. **⚠ Agent loop broken on OpenCode 1.14.48** (0/6 runs emit any AI SDK 5 events) — restore TrevorJS Gemma 4 31B-it Uncensored fallback for working agent main |
