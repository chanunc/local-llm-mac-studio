...
- **lm-studio** (LM Studio headless, port 1234, OpenAI only) — stopped (2026-05-06). Prior main (2026-05-05): `unsloth/granite-4.1-30b-GGUF` Q8_0 (IBM Granite 4.1 30B instruct, Apache 2.0, 28.57 GiB, 24.8 tok/s gen, browse 6.24 s / search 10.51 s). Earlier: TrevorJS Gemma 4 26B A4B Uncensored Q8_0 (browse 2.93 s 🥇). Both on disk, reloadable; guardrail override required for initial load of most models.
...
- **mlx-openai-server / mlx-lm server** (port 8000, OpenAI only) — **current production main (2026-05-06)**: `lmstudio-community/gemma-4-31B-it-MLX-6bit` served via `/opt/homebrew/Cellar/mlx-lm/0.31.3/libexec/bin/mlx_lm.server` (Cellar libexec binary, **not** `/opt/homebrew/bin/mlx_lm.server` — that symlink resolves to a python3.11 mlx_lm install missing Gemma 4 support; **20.4 tok/s, browse 12.33 s / search 35.55 s**, thinking mode ON). MTP drafter end-to-end attempt 2026-05-06 via mlx-vlm 0.5.0 from main (PyPI 0.4.4 lacks `mlx_vlm.speculative`; install includes PRs #1112/#1115/#1117). Drafter at upstream-expected efficiency — 3.07–4.29 acc/round, matches the maintainer's PR #1115 B=1 reference (11.7 tok/s, 4.64 acc/round). Two opencode-bench passes, with and without `MLX_VLM_SPEC_BATCH_WAIT_MS=10`, both 8/8 timeouts. Two distinct blockers diagnosed: (1) mlx-vlm streaming hangs on long-reasoning prompts (trivial prompts stream cleanly, "Browse www.example.com" hangs 360 s; reproducible upstream bug worth filing); (2) bf16 decode at 12.3 tok/s × 8192-token reasoning budget = 666 s/turn, exceeds opencode's 300 s wall regardless. `chat_template.jinja` byte-identical to working 6-bit's — issue [#941](https://github.com/Blaizzy/mlx-vlm/issues/941) ruled out. bf16 also OOMs at 32 K+. Full analysis in [`docs/models/per-model/model-summary-gemma.md`](docs/models/per-model/model-summary-gemma.md#gemma-4-31b-it-bf16--mtp-drafter-mlx-vlm-2026-05-06-failed-experiment). `mlx-openai-server` also available with trie-based prompt caching, speculative decoding, and multi-model YAML config.
...
- **llama-cpp-turboquant** (port 8099, OpenAI only) — provisional **sidecar** built from the [`johndpope/llama-cpp-turboquant`](https://github.com/johndpope/llama-cpp-turboquant) `feature/planarquant-kv-cache` fork. Adds RotorQuant / IsoQuant / TurboQuant / PlanarQuant **KV-cache compression types** (`iso3/4`, `turbo2/3/4`, `planar3/4`) on top of stock llama.cpp Metal. First Apple-Silicon path for RotorQuant. Last exercised 2026-05-06 with `unsloth/Qwen3.6-35B-A3B-UD-Q6_K.gguf` + `--cache-type-k iso3 --cache-type-v iso3`: smoke 5/5, **decode 46.3 tok/s @ 512 (2.27× Gemma 4 baseline)**, but **cold prefill regression at 32 K+ contexts** (>600 s timeout) and **agent loops slower** than Gemma 4 (browse 20.5 s vs 12.33 s, search 151.18 s vs 35.55 s) because OpenCode's tool-result injection is dominated by cold prefill. Kept as documented sidecar — fits short-context / multi-turn workloads where the prompt prefix re-caches cleanly. Full analysis: [`docs/models/techniques/model-technique-rotorquant.md`](docs/models/techniques/model-technique-rotorquant.md) and [`docs/servers/llama-cpp-turboquant/summary.md`](docs/servers/llama-cpp-turboquant/summary.md).

Only one of vllm-mlx, mlx-openai-server, oMLX, or vmlx can hold port 8000 at a time — switch by killing the current process before starting another (and see Event 4 pre-benchmark hygiene before any benchmark run). dflash-mlx (8098) and lm-studio (1234) can coexist with the port-8000 server.
...
MacBook / Linux / WSL  ──── LAN ────>  Mac Studio M3 Ultra (<MAC_STUDIO_IP>)
  ...
  Pi                                     vmlx (JANGTQ) :8000     ┘
                                         ...
                                         OpenAI + Anthropic API native (lm-studio + dflash-mlx: OpenAI only)
```
...
- Save raw JSON output to `docs/models/benchmarks/<model-slug>/<benchmark-type>.json` (or `<benchmark-type>-<server>.json` for cross-server comparisons — see the `agent-bench-lm-studio.json` precedent)
...
**lm-studio configs** (`configs/clients/lm-studio/`) currently ship `opencode.json` only. If lm-studio graduates from provisional to permanent server status (sustained production use), backfill `claude-code-settings.json`, `pi-models.json`, `openclaw-provider.json`, `qwen-code-settings.json` to match the other servers.
