...
8. **Nemotron Cascade 2 + OpenClaw**: Nemotron Cascade 2 uses ChatML format with Qwen3-style tool XML, but the chat template is embedded in tokenizer Python code — not in `tokenizer_config.json` (which is empty). mlx-lm/mlx-openai-server can't invoke this code path, so tools sent by OpenClaw appear as raw text in the prompt and the model echoes back tool XML (`<parameter=path>...`) instead of answering. Additionally, mlx-openai-server lacks the `nemotron_v3` reasoning parser and `qwen3_coder` tool-call parser that vLLM provides. Use Nemotron only with direct-chat clients (Claude Code, OpenCode) that don't inject tools, or via vLLM with `--tool-call-parser qwen3_coder`.
