...
| **[llama-cpp-turboquant](docs/servers/llama-cpp-turboquant/summary.md)** (provisional, :8099) | 🟢 High-decode (small ctx) / 🔴 cold prefill | Single GGUF + RotorQuant / TurboQuant / PlanarQuant KV cache | OpenAI | **First Apple-Silicon path for RotorQuant.** Built from `johndpope/llama-cpp-turboquant` `feature/planarquant-kv-cache` fork. `--cache-type-{k,v} iso3` exposes Clifford-rotor KV cache. On Qwen3.6-35B-A3B Q6_K: smoke 5/5, decode **46.3 tok/s @ 512 (2.27× Gemma 4)**, but cold prefill regression at 32 K+ → OpenCode browse 20.5 s / search 151.18 s (slower than Gemma 4 in agent loops). See [bench](docs/models/benchmarks/qwen36-35b-a3b-rotorquant-iso3/) |
