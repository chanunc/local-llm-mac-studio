...
- **llmster** (LM Studio headless, port 1234, OpenAI only) — **current production main** (2026-05-02): `HauhauCS/Qwen3.6-35B-A3B-Uncensored-HauhauCS-Aggressive` `Q6_K_P` GGUF (MoE 35B/3B + VL, ~31 GB). Fastest end-to-end on agent loops for standard MLX / GGUF models (3–5× faster than vllm-mlx on the prior 27B-6bit precedent; OpenCode browse 5.14 s / search 12.01 s on the Aggressive variant — essentially tied with Gemma 4 31B-it on browse, 2nd on search).
