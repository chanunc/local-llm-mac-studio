...
| **llama-cpp-turboquant** | **8099** | **RotorQuant / TurboQuant / PlanarQuant KV cache** -- built from `johndpope/llama-cpp-turboquant` `feature/planarquant-kv-cache` fork. First Apple-Silicon path for RotorQuant `iso3/4`. **Currently running (2026-05-06):** Qwen3.6-35B-A3B Q6_K + `iso3` KV — smoke 5/5, decode 46.3 tok/s @ 512, but cold prefill regression at 32 K+; OpenCode browse 20.5 s / search 151.18 s (slower than Gemma 4 in agent loops). | unsloth/Qwen3.6-35B-A3B-UD-Q6_K.gguf (~27 GB) | Not needed |
