...
|:--|:--|:--|:--|
| Thai ↔ English translation (port 8080) | `~/mlx-models/chindamt-4b-4bit/` (mlx-lm 0.31.3, OpenAI API) | `iapp/ChindaMT-4B` — Qwen3.5-4B base, hybrid linear-attention (24/32 SSM layers), Thai ↔ English MT, Apache-2.0, MLX 4-bit / 2.2 GB | Deployed 2026-05-15. **186 tok/s decode, 2.5 GB peak memory** — fits alongside any main model on the 96 GB M3 Ultra. Call with `chat_template_kwargs: {"enable_thinking": false}` or output lands in `message.reasoning` instead of `message.content`. GGUF path not viable — llama.cpp has no SSM/linear-attention runtime support. Converted via mlx-lm after stripping tied `lm_head.weight` (bug in mlx-lm qwen3_5 outer sanitize — fixed-hf source at `~/mlx-models/chindamt-4b-fixed-hf/`). Launch: `nohup /opt/homebrew/Cellar/mlx-lm/0.31.3/libexec/bin/mlx_lm.server --model ~/mlx-models/chindamt-4b-4bit --host 0.0.0.0 --port 8080 > /tmp/chindamt.log 2>&1 &`. Stop: `pkill -f 'mlx_lm.server.*chindamt'`. |
